    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args){
        ...
        foreach($this->getPlugin()->getServer()->getOnlinePlayers() as $staffers){
            if($staffers->hasPermission("servermoderator.unban")){
                $staffers->sendMessage($this->getPlugin()->getConfig()->get("prefix") . " Staffer " . $sender->getName() . " unbanned Player " . $args[0]);
            }

        }
        $this->getPlugin()->getServer()->broadcastMessage($this->getPlugin()->getConfig()->get("prefix") . " Player " . $args[0] . "is unbanned." . "by " . $sender->getName());
    }
